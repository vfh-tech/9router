...
🎉 Панель управления откроется на `http://localhost:2222`
...
URL по умолчанию:
- Панель управления: `http://localhost:2222/dashboard`
- OpenAI-совместимый API: `http://localhost:2222/v1`
...
| `PORT` | framework default | Порт сервиса (`2222` в примерах) |
...
| `BASE_URL` | `http://localhost:2222` | Внутренний серверный базовый URL для задач облачной синхронизации |
...
- Установите `PORT=2222` и `NEXT_PUBLIC_BASE_URL=http://localhost:2222`

**Ошибки облачной синхронизации**
- Убедитесь, что `BASE_URL` указывает на ваш работающий инстанс (например, `http://localhost:2222`)
...
- Локально (`http://127.0.0.1:2222/v1/chat/completions`): работает с `stream=false` и `stream=true`.

...
// Cheap pre-parse check for the passthrough lazy gate: does this data-line JSON
// carry an id that fixInvalidId would rewrite? Mirrors fixInvalidId (streamHelpers):
// it fires when id exists but is "chat"/"completion"/short. Here we only need to
// know whether a parse is required — a line with no `"id":` short string literal
// can't trigger it. Matches the quoted short forms fixInvalidId rewrites.
const SHORT_ID_RE = /"id"\s*:\s*"(chat|completion|[a-z0-9-]{0,7})"/;
function dataLineHasValidId(payload) {
  return !SHORT_ID_RE.test(payload);
}
...
export function createSSEStream(options = {}) {
  ...
  // ponytail: O(n²) string += per chunk replaced by chunk arrays joined once at
  // finalize — same result, no quadratic copy over the full response text.
  const contentParts = [];
  const thinkingParts = [];
  let ttftAt = null;
  let sseLineCount = 0;
      ...
      for (const line of lines) {
        ...
        if (mode === STREAM_MODE.PASSTHROUGH) {
          ...
          if (
            ...
            // ponytail: lazy-parse gate — passthrough forwards the raw line
            // verbatim unless the chunk touches one of the fields we patch
            // (usage/finish_reason, id/created/object injection, Azure filter
            // fields, empty tool_calls). Markers are cheap substring checks;
            // false negatives impossible because every patched field name is
            // listed here. Add a marker when a new patch rule is added below.
            const payload = trimmed.slice(5).trim();
            const needsParse =
              payload.includes("usage") ||
              payload.includes("finish_reason") ||
              payload.includes("prompt_filter_results") ||
              payload.includes("content_filter_results") ||
              payload.includes("tool_calls") ||
              !dataLineHasValidId(payload);
            if (needsParse) {
              try {
              ...
              // Gate says no patch needed — the raw line is forwarded unchanged below.
              output = null;
            }
          }
        ...
        if (parsed.delta?.thinking) {
          ...
          thinkingParts.push(parsed.delta.thinking);
        }
        ...
        if (parsed.choices?.[0]?.delta?.reasoning_content) {
          ...
          thinkingParts.push(parsed.choices[0].delta.reasoning_content);
        }

...
export default class TraeExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, proxyOptions = null }) {
    ...
    // renderNewText used to rebuild the full concatenation (order.map+join)
    // per event — O(n²) over long turns. The cached `full` string is appended
    // in O(delta) on the hot path (last item growing); a full rebuild only
    // happens on the rare out-of-order growth case, preserving exact output.
    const order = [];
    const thoughts = {};
    let full = "";
    let fullPid = null;
    ...
    const renderNewText = (data) => {
      ...
      const prev = thoughts[pid] || "";
      if (t.length >= prev.length) {
        if (pid === fullPid) full += t.slice(prev.length);
        thoughts[pid] = t;
      }
      if (fullPid !== pid) {
        fullPid = pid;
        full = order.map((i) => thoughts[i]).join("");
      }
